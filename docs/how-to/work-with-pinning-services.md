...
**Pinning** in IPFS is a process that ensures that the pinned data is exempt from routine [garbage collection](../concepts/persistence.md#garbage-collection) and is therefore always available. In addition to [locally pinning files](pin-files.md), you can use a **remote pinning service**, which abstracts this responsibility away from a local, individual node. The appropriate solution depends on your particular use case.

1. Determine whether to [use an existing service](work-with-pinning-services.md#when-to-use-a-remote-pinning-service) or [create your own](work-with-pinning-services.md#when-to-create-your-own-service).
2. Follow the appropriate steps:
   * [Use an existing service](work-with-pinning-services.md#use-a-third-party-pinning-service)
   * [Create your own service](work-with-pinning-services.md#create-your-own-pinning-service)
...
If you've decided that an existing service is right for you, learn [how to use an existing service](work-with-pinning-services.md#use-a-third-party-pinning-service)
...
::: warning Your service must use the [IPFS Pinning Service API](https://ipfs.github.io/pinning-services-api-spec) in order to be interoperable with clients using [`ipfs pin remote`](../reference/kubo/cli.md#ipfs-pin-remote) commands. :::

If you've decided that creating your own pinning service is right for you, learn [how to create your own pinning service](work-with-pinning-services.md#create-your-own-pinning-service).
...
::: callout As of June 2023, [Filebase](https://filebase.com) and [Pinata](https://pinata.cloud/) support the [IPFS Pinning Service API endpoint](https://github.com/ipfs/pinning-services-api-spec). :::
...
* [IPFS Desktop or IPFS Web UI](work-with-pinning-services.md#ipfs-desktop-or-ipfs-web-ui)
* [IPFS Kubo](work-with-pinning-services.md#ipfs-kubo)
...
First, [add the pinning service](work-with-pinning-services.md#add-a-pinning-service-in-the-ui). Then, you can [use it](work-with-pinning-services.md#use-a-pinning-service-in-the-ui).
...
2. In **Settings** , navigate to **Pinning Services**.
3.  Click the **Add Service** button.

    ![The Desktop/Web UI Settings screen, ready for adding a new pinning service](../../.gitbook/assets/add-service.jpg)
4.  In the pop up, select your chosen pinning service.
    ...
    ![Desktop/Web UI modal for selecting a pinning service to add](../../.gitbook/assets/add-service-picker.jpg)
5.  Next, enter the following information:
    ...
    * You **secret access token**, provided to you by the pinning service. See the your specific pinning service documentation for more info.
    ...
    Example using Pinata and [pinata.cloud/keys](https://app.pinata.cloud/keys): ![Pinata secret access token](../../.gitbook/assets/add-service-pinata-token.jpg)

    Example using Filebase and [console.filebase.com/keys](https://console.filebase.com/keys): ![Filebase secret access token](../../.gitbook/assets/add-service-filebase-token.png)

    Comparison between an existing third-party service and a custom service:

    | Third-party service (e.g., Pinata)                                               | Custom service                                                                                   |
    | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
    | ![Adding Pinata in Desktop/Web UI](../../.gitbook/assets/add-service-pinata.jpg) | ![Adding custom pinning service in Desktop/Web UI](../../.gitbook/assets/add-service-custom.jpg) |
6.  Click **Save**. Your new pinning service is added to the **Pinning Services** section of your **Settings** screen.

    ![Desktop/Web UI Settings screen with a new pinning service added](../../.gitbook/assets/added-service.jpg)

#### Use a pinning service in the UI

Once you've [added a pinning service](work-with-pinning-services.md#add-a-pinning-service-in-the-ui), you can quickly pin or unpin files to your new pinning service directly from the UI.

To pin a file in the UI:

1. Navigate to the **Files** screen.
2. Right-click any file or click the **three dots** action icon in the files list
3.  Select **Set pinning**:

    ![Desktop/Web UI Files screen with the action menu open and "Set pinning" visible](../../.gitbook/assets/set-pinning.jpg)
...
1. Open a terminal.
2.  Use the [`pin remote service add`](https://docs.ipfs.tech/reference/kubo/cli/#ipfs-pin-remote-service-add) command to add the service:
...
Success. You can now [use the pinning service](work-with-pinning-services.md#use-a-pinning-service-in-kubo).
...
2. [Reuse an existing solution](https://github.com/ipfs/pinning-services-api-spec#adoption) to reduce development time.
