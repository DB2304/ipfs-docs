...
description: >-
  Learn about the different ways that CIDs can be fetched from the other peers
  in the IPFS network
---
...
:::callout The CID you will retrieve is actually a folder containing a single image file. The reason for this that when files are added to IPFS, the filename is not stored by default. To retain the filename, it's a common practice to wrap the file in a directory. In such instances, you end up with two CIDs - one for the file and another for the directory containing the file. :::

## Contents

* [IPFS retrieval methods](retrieve.md#ipfs-retrieval-methods)
* [Verified vs. trusted CID retrieval](retrieve.md#verified-vs-trusted-cid-retrieval)
* [Fetching the CID with Kubo](retrieve.md#fetching-the-cid-with-kubo)
* [Fetching the CID with an IPFS Gateway](retrieve.md#fetching-the-cid-with-an-ipfs-gateway)
* [Summary and next steps](retrieve.md#summary-and-next-steps)
